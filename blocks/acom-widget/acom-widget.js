      ...
      if (xhr.readyState === 4 && xhr.status === 201) {
        ...
        try {
          ...
          if (redirect !== 'off') {
            ...
            console.log(
              'Blob Viewer URL:',
              `<a href="${blobViewerUrl}" target="_blank">View PDF</a>`,
            );
          }
        } catch (error) {
...
export default async function init(element) {
  ...
  const statusMessage = createTag(
    'span',
    { class: 'message' },
    'Uploading file to acrobat.adobe.com',
  );
  const statusPercentage = createTag('span', { class: 'percentage' }, '0%');

  const button = createTag('input', { type: 'file', id: 'file-upload', class: 'hide' });
  const cancelButton = createTag(
    'button',
    { class: 'widget-cancel con-button outline button-xl hide' },
    'Cancel',
  );
  const buttonLabel = createTag(
    'label',
    { for: 'file-upload', class: 'widget-button' },
  ...
  const something = createTag('img', {
    class: 'something',
    src: content[5].getElementsByTagName('img')[0].src,
  });
  ...
  actionArea.append(button, cancelButton, buttonLabel, something);
